...
def run_v2(input_dir: Path, output_dir: Path, convex_hull_algorithm_name: str) -> None:
    try:
        hull_algorithm = CONVEX_HULL_ALGORITHMS[convex_hull_algorithm_name]
    except KeyError as error:
        known = ", ".join(sorted(CONVEX_HULL_ALGORITHMS))
    ...
    for case_file in _iter_case_files(input_dir):
        payload = json.loads(case_file.read_text(encoding="utf-8"))
        try:
            result = v2_layered_area_weighted_peeling(payload["points"], convex_hull_algorithm=hull_algorithm)
        except AmbiguousPeelError as error:
            result = {"error": str(error)}
        output_path = output_dir / case_file.name
        output_path.write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    if args.version == "v1":
        ...
        return 0
    if args.version == "v2":
        run_v2(Path(args.input_dir), Path(args.output_dir), args.convex_hull_algorithm)
        return 0
